...
const Root = ({
  ...
  children,
  ...props
}: HTMLAttributes<HTMLDivElement> & RootProps) => {
...
const HeaderComponent = ({
  ...
  hideLabels = false,
  ...props
}: HTMLAttributes<HTMLDivElement> & HeaderProps) => {
  ...
  useEffect(() => {
    ...
    if (activeTabRef && tabIndicatorRef.current) {
      ...
      tabIndicatorRef.current.style.left = prevTabRef
        ? `${
            tabRefs.current.slice(0, activeTabIndex).reduce((total, tab) => total + (tab?.offsetWidth || 0) + 12, 0) +
            15
          }px`
        : "15px";
    }
  ...
  return (
    <div>
      <div className={cn("flex items-start px-4 pt-2.5", className)} {...props}>
        {!hideLabels &&
      ...
      <div className="w-full px-0.5">
        <div className="bg-ui-contrast-border-top relative h-px w-full">
          <div
            ref={tabIndicatorRef}
            className={cn("absolute bottom-0 transition-all motion-reduce:transition-none", "duration-150 ease-linear")}
          >
...
const Meta = ({ className, ...props }: HTMLAttributes<HTMLDivElement>) => {
  return <div className={cn("txt-compact-small text-ui-contrast-fg-secondary ml-auto", className)} {...props} />;
};
Meta.displayName = "CodeBlock.Header.Meta";

const Header = Object.assign(HeaderComponent, { Meta });

const Body = ({ className, children, ...props }: HTMLAttributes<HTMLDivElement>) => {
          ...
          <div className="max-w-[90%]">
            ...
            >
              {({ style, tokens, getLineProps, getTokenProps }) => (
                <pre
                  className={cn("code-body whitespace-pre-wrap bg-transparent", {
