import { type CalendarDate, createCalendar, getLocalTimeZone } from "@internationalized/date";
import { TriangleLeftMini, TriangleRightMini } from "@medusajs/icons";
import type { SetStateAction, Dispatch } from "react";
import { useState, useMemo, useEffect } from "react";
import { type DateValue, useCalendar, useLocale, type CalendarProps as BaseCalendarProps } from "react-aria";
...
const Calendar = (props: CalendarProps) => {
  const [value, setValue] = useState<CalendarDate | null | undefined>(() =>
    getDefaultCalendarDate(props.value, props.defaultValue),
  );

  const { locale } = useLocale();
  const _props = useMemo(() => convertProps(props, setValue), [props]);
...
function convertProps(
  props: CalendarProps,
  setValue: Dispatch<SetStateAction<CalendarDate | null | undefined>>,
